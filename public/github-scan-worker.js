...
function deepSet(obj, path, value, throwOnUndefined = true) {
  let current = obj;
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (current[key] === void 0 || current[key] === null) {
      if (throwOnUndefined) {
        throw new Error(
          `Property "${key}" does not exist and force is set to true.`
        );
      }
      current[key] = {};
    }
  ...
  if (throwOnUndefined && !(lastKey in current)) {
    throw new Error(
      `Property "${lastKey}" does not exist and force is set to true.`
    );
  }
...
function getRemainingNumberOfRequests(
  ...
  const { totalCount, nodes } = deepGet(currentData, paginator.path);
...
function mergeData(currentData, paginators, newData) {
  if (currentData) {
    for (const paginator of paginators) {
      ...
      const { nodes } = deepGet(newData, paginator.path);
      const { nodes: currentNodes } = deepGet(currentData, paginator.path);
      deepSet(currentData, paginator.path, {
        ...deepGet(currentData, paginator.path),
        nodes: [...currentNodes, ...nodes],
      });
      if (newData.rateLimit) {
        deepSet(currentData, ["rateLimit"], newData.rateLimit);
      }
    }
  } else {
    currentData = newData;
...
function updatePaginators(
  ...
  for (const paginator of paginators) {
    ...
    } = deepGet(data, paginator.path);
    const { nodes } = deepGet(currentData, paginator.path);
...
async function paginateQuery(args) {
  ...
  const newData = await client(query, variables);
...
async function* graphqlFetchAll(args) {
  let { url, headers, query, variables, paginators, currentData, maxPerPage } =
    args;
  maxPerPage = maxPerPage || DEFAULT_MAX_PER_PAGE;
